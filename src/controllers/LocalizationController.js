...
class LocalizationController {
  ...
  async _getMeets(beaconsIds, devicesMacs) {
    ...
    extDate = extDate.setSeconds(
      extDate.getSeconds() - process.env.LOCALIZATION_INTERESTING_SEC
    );
  ...
  async locationData(beaconsIds, devicesMacs) {
    ...
    for (const [key, value] of Object.entries(devices)) {
      value.meets = value.meets.sort((a, b) =>
  ...
  async populateBeacons(localization) {
    const beacons = await OrganizationController.getOrgBeacons(
  ...
  async getLocalizationDevicesData(localization, beaconIds) {
    if (process.env.FILTER_ONLY_ORG_DEVICES) {
      const devices = (
        await DeviceController.getOrgDevices(localization.organizationId)
  ...
  async populateDeviceNames(localization) {
    for (const device of localization.devices) {
      device.name =
        (await DeviceController.getDeviceByMac(device.mac))?.name || "";
    }
  ...
  async localize(localization) {
    ...
    localization.customLocalizationData.caclulatingTimes.populatePlan =
      new Date() -
      startTime -
      localization.customLocalizationData.caclulatingTimes.total;
    localization.customLocalizationData.caclulatingTimes.total =
      new Date() - startTime;
    ...
    let localizationDevices = await this.getLocalizationDevicesData(
      localization,
      beaconIds
    );
    ...
    switch (localization.type) {
      case "NEAREST_FINGERPRINT":
        localization.devices = await NearestFingerPrint.localize(
          localization._id,
          localizationDevices
        );
  ...
  async localizeAll(socket) {
    ...
    for (let i = 0; i < localizations.length; i++) {
      ...
      await CacheController.store(
        "loc." + localization.organizationId + "." + localization._id,
        localization
      );
    }
  }
