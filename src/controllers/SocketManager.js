  ...
  init(httpServer) {
    ...
    io.on("connection", async (socket) => {
      ...
      socket.on("auth", async (auth) => {
        ...
        const localization = await LocalizationController.getById(
          auth.localizationId
        );

        if (
          localization &&
          localization.organizationId === userData.organization._id
        ) {
      ...
      socket.on("localization-update", async (data) => {
        if (
          data.secretToken === process.env.COMPUTING_NODE_TOKEN &&
          data.localization
        ) {
