...
class CalculatePos {
  getOnlyRelativeDevices(devices) {
    return devices.filter((dev) => {
      dev.lastSeens = dev.lastSeens.filter(
        (ls) => Time.diffInSeconds(new Date(ls.date)) < 30
      );
      ...
      if (dev.data.length >= 3) {
        ...
        dev.data.forEach((d) => {
          data.push({
            x: d.beacon.x,
            y: d.beacon.y,
            distance: this.rssiToDistance(d.rssi),
          });
        });

        dev.pos = {
          ...trilateration.calculate(data),
          p: "*",
      ...
      prints.forEach((print) => {
        ...
        print.beacons.forEach((beacon) => {
          if (deviceBeacons.includes(beacon.deviceKey)) {
            const deviceRssi = dev.lastSeens.find(
              (ls) => ls.deviceKey === beacon.deviceKey
            ).rssi;
